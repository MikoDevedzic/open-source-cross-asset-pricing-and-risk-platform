// restore_pricer.js
// Run: node C:\Users\mikod\OneDrive\Desktop\Rijeka\restore_pricer.js

const fs = require('fs');
const PATH = 'C:\\Users\\mikod\\OneDrive\\Desktop\\Rijeka\\backend\\api\\routes\\pricer.py';
...
def _build_curve(ci: CurveInput, valuation_date: date, db: Session) -> Curve:
    ...
    try:
        ...
        if row and row.quotes:
            ...
            print(f"[PRICER] DB snapshot for '{ci.curve_id}': {len(db_quotes)} quotes, first={db_quotes[0] if db_quotes else None}")
            if len(db_quotes) >= 2:
                return bootstrap_from_dicts(valuation_date, db_quotes)
            else:
                print(f"[PRICER] Not enough quotes to bootstrap: {len(db_quotes)}")
        else:
            print(f"[PRICER] No snapshot found in DB for curve_id='{ci.curve_id}'")
...
console.log('Done. pricer.py restored with debug prints.');
console.log('Now price a trade and watch the backend terminal for [PRICER] lines.');
