...
function PricingPanel({ trade }) {
  const { priceTrade, resultsByTrade, loading, error } = usePricerStore()
  const result = resultsByTrade[trade.id]
  ...
  useEffect(() => { initCurveState(curveIds) }, [curveIds.join(',')]) // eslint-disable-line
  ...
  const handlePrice = async () => {
    setLocalErr(null)
    // Sprint 4E: validation handled in CurveInputPanel
    const parsed = buildCurveInputs(curveIds)
    // Sprint 4E: buildCurveInputs handles default fallback
