...
// ── useAuthStore.js ───────────────────────────────────────────
write('frontend/src/store/useAuthStore.js',
...
// ── SignupPage.jsx ────────────────────────────────────────────
write('frontend/src/components/auth/SignupPage.jsx',
`import { useState } from 'react'
    ...
    <header className="appbar">
      <div className="appbar-left">
        <span className="appbar-logo"
          onClick={() => navigate('/configurations/market-data/curves')}
    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/configurations/*"
          element={<AuthGuard><ConfigurationsLayout /></AuthGuard>} />
        <Route path="/pricer"
          element={<AuthGuard><div className="app-root"><AppBar /><StubPage title="Pricer" sprint={5} /></div></AuthGuard>} />
        <Route path="/pnl"
          element={<AuthGuard><div className="app-root"><AppBar /><StubPage title="PnL Attribution" sprint={11} /></div></AuthGuard>} />
        <Route path="/market-risk"
          element={<AuthGuard><div className="app-root"><AppBar /><StubPage title="Market Risk" sprint={10} /></div></AuthGuard>} />
        <Route path="/ccr"
          element={<AuthGuard><div className="app-root"><AppBar /><StubPage title="CCR" sprint={14} /></div></AuthGuard>} />
        <Route path="/simm"
          element={<AuthGuard><div className="app-root"><AppBar /><StubPage title="ISDA SIMM" sprint={16} /></div></AuthGuard>} />
        <Route path="/blockchain"
          element={<AuthGuard><div className="app-root"><AppBar /><StubPage title="Blockchain" sprint={18} /></div></AuthGuard>} />
...
.auth-card{width:100%;max-width:400px;background:var(--panel);border:1px solid var(--border);padding:2.5rem 2rem;display:flex;flex-direction:column;align-items:center}
...
const indexCssPath = path.join(base, 'frontend/src/index.css')
fs.appendFileSync(indexCssPath, authCss, 'utf8')
console.log('✓ frontend/src/index.css (appended auth styles)')

// ── .gitignore ────────────────────────────────────────────────
const giPath = path.join(base, '.gitignore')
